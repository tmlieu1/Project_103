        ...
        for item in value:
            ...
            if item[2] == user_year and item[0] == 1:
                ...
                if item[1] == 'Girl':
                    ...
                    if len(key) > max_girl:
                        max_girl = len(key) - len(key) % 8
    
    helper_unique(girl_list, max_girl, "Girls")
    helper_unique(boy_list, max_boy, "Boys")
    
def helper_unique(gen_list, max_gen, gender):
    count = 0
    ...
    for gen in gen_list:
        ...
        for num in range(max_gen//8, 0, -1):
            
            #if len name greater than num but less than num +1,
            #and difference between boy and num + 1 < 8
            #tab = tab + 1
            if len(gen) >= (num * 8) and len(gen) < ((num + 1) * 8)\
               and (((num + 1) * 8) - len(gen)) <= 8:
                
                print(gen, end = "\t" + "\t" * (max_gen//8 - num))
            
            if len(gen) < 8 and ((num * 8) - len(gen)) <= 8:
                
                print(gen, end = "\t" + "\t" * (max_gen//8))
...
def longest_names(names_dict):
    ...
    for key, value in names_dict.items():
        ...
        if "-" in key:
            ...
            if len(key) > len(hyp_girl[0]) and value[0][1] == "Girl":
                hyp_girl = [key, value[0][2]]
                
            if len(key) > len(hyp_boy[0]) and value[0][1] == "Boy":
                hyp_boy = [key, value[0][2]]
        
        else:
            
            if len(key) > len(unhyp_girl[0]) and value[0][1] == "Girl":
                unhyp_girl = [key, value[0][2]]
                
            if len(key) > len(unhyp_boy[0]) and value[0][1] == "Boy":
                unhyp_boy = [key, value[0][2]]            
            
    print(f"The longest name given to a baby girl between 1980 and 2017 was {hyp_girl[0]} in {hyp_girl[1]}")
    print(f"The longest non-hyphenated name given to a baby girl between 1980 and 2017 was {unhyp_girl[0]} in {unhyp_girl[1]}")
    print()
    print(f"The longest name given to a baby boy between 1980 and 2017 was {hyp_boy[0]} in {hyp_boy[1]}")
    print(f"The longest non-hyphenated name given to a baby boy between 1980 and 2017 was {unhyp_boy[0]} in {unhyp_boy[1]}")
    return
...
def main():
    ...
    while user_choice != "0" or user_choice == "" or user_choice == None:
        ...
        if user_choice == "7":
            longest_names(dict_names)

        if user_choice == "8":
            wildcard_search(dict_names)
